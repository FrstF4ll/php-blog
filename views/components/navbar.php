    ...
    <div class="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
        <div class="relative flex h-16 items-center justify-between">
            ...
            <?php if (!empty($_SESSION['id'])): ?>
                <form action="?pages=logout" method="POST" class="text-white right-0 flex items-center gap-3 pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
                    <p><?= $_SESSION['name'] ?></p>
                    <button type="submit" class="text-sm font-medium text-red-800 bg-red-200 hover:bg-red-100 px-4 py-2 rounded-lg transition-colors">Log out</button>
                </form>
            <?php else: ?>
